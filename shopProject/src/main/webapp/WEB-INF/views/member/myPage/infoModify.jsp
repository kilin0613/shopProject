...
<body>
	...
	<form:form modelAttribute="memberVo" action="infoModify" method="post"
	onsubmit="return validateSignUp()">
		<p>
			<label>아이디</label>
			<form:input path="id" class="btn_info" id="id"
				value="${loginMember.id}" readonly="true" />
			<label style="font-size: 8pt">*변경불가항목</label>
		</p>
		<p>
			<label>비밀번호</label>
		...
		<p>
			<label>성별</label>
			<form:radiobutton path="sex" value="M" label="남자" checked="${loginMember.sex}" disabled="true"/>
			<form:radiobutton path="sex" value="W" label="여자" checked="${loginMember.sex}" disabled="true"/>
			<label style="font-size: 8pt">*변경불가항목</label>
		</p>
		<p>
			<label>주소</label>
		...
		<form:input path="customer_id" type="hidden" value="${loginMember.customer_id}"/>
